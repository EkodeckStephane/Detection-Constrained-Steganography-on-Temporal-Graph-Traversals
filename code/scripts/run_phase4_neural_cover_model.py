...
def main() -> None:
    ...
    from models.neural_sequence import metrics_to_dict, train_and_evaluate

    config = _load_yaml(ROOT / "experiments/real_world/phase4_neural_cover_model.yaml")
    ...
    for dataset_id in config["datasets"]:
        ...
        for kind in config["models"]:
            metrics = train_and_evaluate(frame, kind=kind, **config["training"])
            summary["datasets"][dataset_id][kind] = metrics_to_dict(metrics)
            for split, values in summary["datasets"][dataset_id][kind].items():
                rows.append({"dataset": dataset_id, "model": kind, "split": split, **values})

    output_dir = ROOT / "results/tables"
    json_output = output_dir / "phase4_neural_cover_model.json"
    csv_output = output_dir / "phase4_neural_cover_model.csv"
